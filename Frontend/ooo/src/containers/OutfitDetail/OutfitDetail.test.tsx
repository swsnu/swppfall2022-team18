...
const stubInitialOutfitStateHaveUC: OutfitState = {
	outfits: [
		{
			id: 1,
			outfit_info: "",
...
const mockStoreWithUC = getMockStore({
	userCloth: stubInitialUserClothState,
	outfit: stubInitialOutfitStateHaveUC,
});
...
describe("<OutfitDetail/>", () => {
	...
	let outfitdetailWithUC: JSX.Element;
	beforeEach(() => {
		jest.clearAllMocks();
		outfitdetail = (
		...
		outfitdetailWithUC = (
			<Provider store={mockStoreWithUC}>
				<MemoryRouter>
					<Routes>
						<Route path="/" element={<OutfitDetail />} />
					</Routes>
	...
	it("should handle clickClothHandler", async () => {
		...
		render(outfitdetailWithUC);
		const clothImagesWithUC = screen.getAllByTestId("samplecloth-image");
