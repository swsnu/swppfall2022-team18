...
import { fetchOutfits, fetchOutfit, fetchFilteredOutfits, fetchSampleCloth } from "./outfit";
...
describe("outfit reducer", () => {
    ...
    it("should handle fetchFilteredOutfit", async() => {
        ...
        await store.dispatch(fetchFilteredOutfits());
        expect(store.getState().outfit.outfits).toEqual([fakeOutfit])
    })

    it("should handle fetchOutfit", async() => {
        axios.get = jest.fn().mockResolvedValue({ data: {outfit: fakeOutfit, sampleclothes: [fakeCloth]}});
