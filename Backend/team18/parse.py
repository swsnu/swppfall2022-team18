...
def csv_to_db(file_name):
    if file_name == 'codi_data.csv':
        ...
        df = open(file_name, 'r', encoding='utf-8-sig')
        dic_list = csv.DictReader(df)
        df_codi = open('codi_data.csv','r')
        outfits = csv.DictReader(df_codi)
        for dic in dic_list:
            label = LabelSet.get_or_create(type = dic['cloth_type'], color = dic['cloth_color'], pattern = dic['cloth_pattern'])
            label.save()
            outfit_list = []
            for outfit in outfits:
                for c in outfit['cloth_links']:
                    if any(dic['cloth_link'] == c):
                        outfit_list.append(Outfit.objects.get(purchase_link = c['codi_link']))
            sample_cloth = SampleCloth(name = dic['cloth_name'], image_link = dic['cloth_image'], purchase_link = dic['cloth_link'], outfit = outfit_list, type = dic['cloth_type'], color = dic['cloth_color'], pattern = dic['cloth_pattern'], label_set = label)
            sample_cloth.save()
    ##이슈
